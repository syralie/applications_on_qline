...
def reader_bob(mode, num_batches, batch_size, path_config):
    ...
    try:
        with open(angle_path, "rb") as af, open(result_path, "rb") as rf:
            ...
            for _ in range(num_batches):
                ...
                if (len(angle_data) != batch_size):
                    logging.info(f"[Reader B] data is of length {len(angle_data)}")
                if (len(result_data) != 2*batch_size):
...
if __name__ == "__main__":
    ...
    angles, results = reader_bob(num_batches=args.num_batches, mode=args.mode, batch_size=args.batch_size, path_config=args.config)
    print_idx = 20
