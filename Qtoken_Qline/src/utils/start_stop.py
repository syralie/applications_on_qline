...
async def send_stop_command(mode: str, path_config: str, reader, writer) -> None:
    ...
    try:
        ...
        logging.warning(
            f"[C] BrokenPipeError while sending stop command. Server connection was already closed: {e}")

    except (ConnectionRefusedError, FileNotFoundError, OSError) as e:
        # Handles connection issues other than Broken Pipe (server not started, socket missing, etc.)
        logging.error(
            f"[C] Failed to send stop command: {e} with socket {socket_path}")
        ...
        if writer is not None:
            ...
            try:
                ...
                logging.warning(
                    "[C] Timeout while waiting for writer to close. Proceeding.")
...
async def send_start_command(mode: str, path_config=None):
    """Send a start command to the appropriate UNIX socket and return reader, writer."""

    if mode in ("hwsim", "test"):
        socket_path = "/tmp/gc_startstop"
    elif mode == "real":
        socket_path = "/home/vq-user/start_stop.socket"
    else:
    ...
    try:
        ...
        logging.error(
            f"[C] Failed to send start command: {e} with socket {socket_path}")
        if writer is not None:
            writer.close()
